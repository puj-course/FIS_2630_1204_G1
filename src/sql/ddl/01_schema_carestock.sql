...
CREATE TABLE IF NOT EXISTS LOTES (
    ...
    CONSTRAINT fk_lotes_medicamento FOREIGN KEY (id_medicamento) REFERENCES MEDICAMENTOS (id_medicamento),
    CONSTRAINT fk_lotes_ubicacion FOREIGN KEY (id_ubicacion) REFERENCES UBICACIONES (id_ubicacion),
    CONSTRAINT ck_lotes_estado CHECK (estado_lote IN ('DISPONIBLE', 'AGOTADO', 'VENCIDO', 'RETENIDO')),
    CONSTRAINT ck_lotes_cantidad CHECK (cantidad_actual >= 0),
    CONSTRAINT uq_lotes_numero UNIQUE (numero_lote, id_medicamento)
);
...
CREATE INDEX IF NOT EXISTS ix_log_fecha ON LOG_MOVIMIENTOS (fecha_hora);
CREATE INDEX IF NOT EXISTS ix_medicamentos_nombre ON MEDICAMENTOS (nombre_comercial);
